...
<head>
    ...
    <title>Rapport de Couverture Pédagogique - Par Département</title>
    <style>
        body {
...
<body>
    <h1>Rapport de Couverture Pédagogique - Par Département</h1>
        ...
        <tbody>
            @forelse($data as $department)
                <tr>
                    ...
                    <!-- Heures -->
                    <td style="text-align: center;">{{ $department->total_planned['hours'] ?? 0 }}</td>
                    <td style="text-align: center;">{{ $department->total_done['hours'] ?? 0 }}</td>
                    <td class="percentage 
                        @php
$pct = ($department->total_planned['hours'] ?? 0) > 0 
                    ...
                    <!-- Leçons -->
                    <td style="text-align: center;">{{ $department->total_planned['lesson'] ?? 0 }}</td>
                    <td style="text-align: center;">{{ $department->total_done['lesson'] ?? 0 }}</td>
                    <td class="percentage 
                        @php
$pct = ($department->total_planned['lesson'] ?? 0) > 0 
                                ? (($department->total_done['lesson'] ?? 0) / $department->total_planned['lesson']) * 100 
                                : 0; @endphp
                        @if ($pct >= 80) high @elseif($pct >= 50) medium @else low @endif"
                        style="text-align: center;">
                        {{ number_format($pct, 2) }}%
                    ...
                    <!-- Leçons Digitalisées -->
                    <td style="text-align: center;">{{ $department->total_planned['lesson_dig'] ?? 0 }}</td>
                    <td style="text-align: center;">{{ $department->total_done['lesson_dig'] ?? 0 }}</td>
                    <td class="percentage 
                        @php
$pct = ($department->total_planned['lesson_dig'] ?? 0) > 0 
                                ? (($department->total_done['lesson_dig'] ?? 0) / $department->total_planned['lesson_dig']) * 100 
                                : 0; @endphp
                        @if ($pct >= 80) high @elseif($pct >= 50) medium @else low @endif"
                        style="text-align: center;">
                        {{ number_format($pct, 2) }}%
                    ...
                    <!-- TP -->
                    <td style="text-align: center;">{{ $department->total_planned['tp'] ?? 0 }}</td>
                    <td style="text-align: center;">{{ $department->total_done['tp'] ?? 0 }}</td>
                    <td class="percentage 
                        @php
$pct = ($department->total_planned['tp'] ?? 0) > 0 
                    ...
                    <!-- TP Digitalisés -->
                    <td style="text-align: center;">{{ $department->total_planned['tp_dig'] ?? 0 }}</td>
                    <td style="text-align: center;">{{ $department->total_done['tp_dig'] ?? 0 }}</td>
                    <td class="percentage 
                        @php
$pct = ($department->total_planned['tp_dig'] ?? 0) > 0 
                                ? (($department->total_done['tp_dig'] ?? 0) / $department->total_planned['tp_dig']) * 100 
                                : 0; @endphp
                        @if ($pct >= 80) high @elseif($pct >= 50) medium @else low @endif"
                        style="text-align: center;">
                        {{ number_format($pct, 2) }}%
