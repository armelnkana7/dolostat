    ...
    <tbody>
        @forelse($data as $schoolClass)
            <tr>
                ...
                <!-- Heures -->
                <td>{{ $schoolClass->total_planned['hours'] ?? 0 }}</td>
                <td>{{ $schoolClass->total_done['hours'] ?? 0 }}</td>
                <td>
                    @php
                        $percentage =
                            ($schoolClass->total_planned['hours'] ?? 0) > 0
                                ? (($schoolClass->total_done['hours'] ?? 0) / $schoolClass->total_planned['hours']) *
                                    100
                                : 0;
                    @endphp
                ...
                <!-- Leçons -->
                <td>{{ $schoolClass->total_planned['lesson'] ?? 0 }}</td>
                <td>{{ $schoolClass->total_done['lesson'] ?? 0 }}</td>
                <td>
                    @php
                        $percentage =
                            ($schoolClass->total_planned['lesson'] ?? 0) > 0
                                ? (($schoolClass->total_done['lesson'] ?? 0) / $schoolClass->total_planned['lesson']) *
                                    100
                                : 0;
                    @endphp
                    {{ number_format($percentage, 2) }}%
                ...
                <!-- Leçons Digitalisées -->
                <td>{{ $schoolClass->total_planned['lesson_dig'] ?? 0 }}</td>
                <td>{{ $schoolClass->total_done['lesson_dig'] ?? 0 }}</td>
                <td>
                    @php
                        $percentage =
                            ($schoolClass->total_planned['lesson_dig'] ?? 0) > 0
                                ? (($schoolClass->total_done['lesson_dig'] ?? 0) /
                                        $schoolClass->total_planned['lesson_dig']) *
                                    100
                                : 0;
                    @endphp
                    {{ number_format($percentage, 2) }}%
                ...
                <!-- TP -->
                <td>{{ $schoolClass->total_planned['tp'] ?? 0 }}</td>
                <td>{{ $schoolClass->total_done['tp'] ?? 0 }}</td>
                <td>
                    @php
                        $percentage =
                            ($schoolClass->total_planned['tp'] ?? 0) > 0
                ...
                <!-- TP Digitalisés -->
                <td>{{ $schoolClass->total_planned['tp_dig'] ?? 0 }}</td>
                <td>{{ $schoolClass->total_done['tp_dig'] ?? 0 }}</td>
                <td>
                    @php
                        $percentage =
                            ($schoolClass->total_planned['tp_dig'] ?? 0) > 0
                                ? (($schoolClass->total_done['tp_dig'] ?? 0) / $schoolClass->total_planned['tp_dig']) *
                                    100
                                : 0;
                    @endphp
                    {{ number_format($percentage, 2) }}%
